...
def img2label_paths(img_paths, anno_path=''):
    ...
    anno_files = {}  # anno files
    for p in anno_path if isinstance(anno_path, list) else [anno_path]:
        p = Path(p)  # os-agnostic
        if p.is_dir():  # dir
            for file in glob.glob(str(p / "**" / "*.txt"), recursive=True):
                file = Path(file)
                stem = file.stem.lower()
                anno_files[stem] = str(file.absolute())
        elif p.is_file():  # file
            with open(p) as t:
            ...
            for line in alllines:
                ...
                stem = file.stem.lower()
                anno_files[stem] = str(file.absolute())
    label_files = [anno_files.get(Path(img_file).stem.lower(), '') for img_file in img_paths]
    return label_files
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        self.im_files, self.label_files = [], []
        try:
            datas = load_all_datas(path)
            self.im_files = list(datas.keys())
            self.label_files = list(datas.values())
            assert self.im_files, f"{prefix}No images found"
        except Exception as e:
            raise Exception(f"{prefix}Error loading data from {path}: {e}\n{HELP_URL}") from e

        cache_dir = Path(os.getcwd()) if not log_dir else Path(log_dir).parent  # 如果有log_dir则为project_dir文件夹
        cache_path = cache_dir/(task_name+".cache")
        try:
        ...
        self.label_files = [datas[img_path] for img_path in self.im_files]  # update
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(
                pool.imap(verify_image_label, zip(self.im_files, self.label_files, repeat(self.class_names))),
                desc=desc,
    ...
    def __getitem__(self, index):
        ...
        if nl:
            ...
            # mqr 筛选scale小于0.02的
            mask = torch.sqrt(labels_out[:, 3] * labels_out[:, 4]) > 0.04
...
def verify_image_label(args):
    ...
    im_file, lb_file, class_names = args
    ...
    try:
        ...
        nc = 1
        msg = f"WARNING ⚠️ {im_file} {lb_file}: ignoring corrupt image/label: {e}"
